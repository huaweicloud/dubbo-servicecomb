...
cse:
  v2:
    test:
      priority: v2
```
  * 服务级配置：priority3.yaml，微服务性选择cse-v2-test-consumer。类型为 yaml。 
```yaml
cse:
  v2:
    test:
...
* 修改
  * priority3.yaml。label信息： app=cse-v2,service=cse-v2-test-consumer,environment=development 。类型为 yaml。 
```yaml
cse:
  v2:
    test:
